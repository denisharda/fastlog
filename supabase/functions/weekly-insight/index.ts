...
serve(async (req) => {
  ...
  try {
    ...
    const body: WeeklyInsightRequest = await req.json();
    const { userId } = body;
    ...
    const prompt = `The user ${profile?.name ?? 'has'} completed ${completedFasts} out of ${totalFasts} planned fasts this week, totaling ${totalHours.toFixed(1)} hours of fasting.
